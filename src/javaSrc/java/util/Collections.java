...
public class Collections {
     ...
     *
     * <p>
     *     在一个 list 中使用二分搜索算法查找指定的元素。该 list 必须以自然升序
     * {@linkplain Comparable natural ordering}的方式排序，否则将可能找不到结果。
     * 在list存在多个与指定元素相等的情况下，无法保证哪个元素被查找到。
     *</p>
     ...
     * <p>
     *     如果 list 实现了 {@link RandomAccess} 接口或者 list 的长度小于 BINARYSEARCH_THRESHOLD
     *     （值为5000的常量），则调用基于索引的二分搜索算法，时间复杂度为 log(n)；
     *     否则，调用基于迭代器的二分搜索算法，复杂度为 O(n) + O(long n)， 即 O(n)。
     * </p>
     *
     * @param  <T> the class of the objects in the list
     ...
     *         <br><p>
     *             如果查找到了指定元素，则返回元素索引；否则返回元素插入点的相反数减 1。
     *             举个例子 ：在 <code>[1, 2, 4]</code> 中查找元素 <code>3</code>，显然
     *             找不到的，那么返回元素 <code>3</code> 插入点（<code>2</code>与<code>4</code>之间）
     *             的相反数减 1，也就是<code>-2 - 1 = -3</code>
     *         </p><br>
     *         <p>
     *             这样返回未找到元素的目的是保证找到元素的索引总是大于或等于0，同时也方便将元素插入
     *             到list中。
     *         </p>
     * @throws ClassCastException if the list contains elements that are not
